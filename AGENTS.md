...
This file provides guidance to AI coding agents (Claude Code, Cursor, Copilot, Antigravity, Devin, etc.) when working with code in this repository.

> **Scope:** This file configures agents working on the `app_Colaciones` repository. The skills in `skills/` are reusable engineering workflows (from [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills)) that govern *how* work is done here.

## Repository Overview

`app_Colaciones` — aplicación para gestión de colaciones. El repo incluye un catálogo de skills (workflows para agentes) en `skills/`, personas en `agents/`, slash commands en `.claude/commands/` y referencias en `references/`. Estos skills extienden las capacidades de los agentes de IA que trabajen en este proyecto.
